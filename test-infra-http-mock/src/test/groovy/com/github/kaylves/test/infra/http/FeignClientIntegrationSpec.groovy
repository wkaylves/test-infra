...
class FeignClientIntegrationSpec extends BaseWireMockSpec {
    ...
    def setup() {
        client = feignClient(UserFeignClient)
    }
    ...
    def "GET - 预设响应后调用"() {
        given:
        willReturn(UserFeignClient, "getUser")
    ...
    def "GET - 列表查询，自动 stub"() {
        given:
        willReturn(UserFeignClient, "listUsers")
    ...
    def "POST - 创建用户"() {
        given:
        willReturn(UserFeignClient, "createUser")
    ...
    def "PUT - 更新用户"() {
        given:
        willReturn(UserFeignClient, "updateUser")
    ...
    def "DELETE - 删除用户"() {
        given:
        willReturn(UserFeignClient, "deleteUser")
    ...
    def "POST Form - 表单提交"() {
        given:
        willReturn(UserFeignClient, "createUserForm")
    ...
    def "POST XML - XML 请求体"() {
        ...
        willReturn(UserFeignClient, "createUserXml").response(xmlResponse)
    ...
    def "GET XML - XML 响应"() {
        ...
        willReturn(UserFeignClient, "getUserXml").response(xmlResponse)
    ...
    def "POST Text - 纯文本请求体"() {
        given:
        willReturn(UserFeignClient, "createUserText")
    ...
    def "GET Text - 纯文本响应"() {
        given:
        willReturn(UserFeignClient, "getUserText")
